...
The discovery layer now globs the wildcard's value against the package directory, derives the captured portion of each match, and substitutes it back into the key pattern to synthesize one concrete `PackageEntry` per file (single-segment `*` only; multi-segment nested wildcards remain unhandled and skip silently). Expanded entries are sorted by subpath so the resulting `snapi.snapshot.json` is byte-stable across runners (`fs.globSync` ordering is not guaranteed). Concrete keys still win over wildcard-expanded ones if both produce the same subpath. Both the wildcard key itself and individual expanded subpaths can still be excluded via `ignoreSubpaths`.
